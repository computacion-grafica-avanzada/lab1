...
vector<Photon*>* PhotonMap::getNeighbourhood(const gdt::vec3f& spoint,
					     const gdt::vec3f* snormal,
					     float          maxdist,
					     unsigned       nphotons) const 
{
 ...
 //   if (np->found < 8) {
	//	std::cout << "menos de 8" << std::endl;
	//delete np;
	//return NULL;
 //   }

    vector<Photon*> * ret = new vector<Photon*>();
	std::cout << "fotones" << std::endl;
	for (int i = 0; i < np->numPhotons(); i++) {
		std::cout << (*np)(i) << std::endl;
		if (snormal) {
			if (gdt::dot(*snormal,(*np)(i).getDirection()) > 0.) {
			ret->push_back(&(*np)(i));
			}
		} else {
			ret->push_back(&(*np)(i));
...
bool PhotonMap::getIrradiance(const gdt::vec3f& spoint,
			      const gdt::vec3f* snormal,
			      float          maxdist,
			      unsigned       nphotons,
			      gdt::vec3f&       irrad) const
{
    irrad = gdt::vec3f(0.0, 0.0, 0.0);
    ...
    for (int i = 0; i < np->numPhotons(); i++) {
		if (snormal) {
			if (gdt::dot(*snormal, (*np)(i).getDirection()) > 0.) irrad + (*np)(i)._power;
		} else {
			irrad + (*np)(i)._power;
		}
    }
	irrad * gdt::vec3f(1. / (M_PI * maxdist * maxdist));
    delete np;
    return true;
}
