...
class PhotonMap {
	 ...
	 */
	virtual vector<Photon*>* getNeighbourhood(const gdt::vec3f& spoint,
	 ...
	 */
	virtual bool getIrradiance(const gdt::vec3f& spoint,
				   const gdt::vec3f* snormal,
				   float          maxdist,
				   unsigned       nphotons,
...
class PhotonMap::NearestPhotons {
    ...
    public:
	NearestPhotons(unsigned n, const gdt::vec3f p, float maxDist,
	...
	gdt::vec3f pointLocation() const { return gdt::vec3f(pos[0],pos[1],pos[2]); }
