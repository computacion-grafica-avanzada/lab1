...
class Photon {
	 ...
	 */
	Photon (const gdt::vec3f& pos, const gdt::vec3f& dir, const gdt::vec3f& pow);
	 ...
	 */
	gdt::vec3f getDirection () const {
	    return gdt::vec3f(SIN_THETA[_theta] * COS_PHI[_phi],
	 ...
	 */
	gdt::vec3f getLocation () const { return gdt::vec3f(_position[0],
						      _position[1],
						      _position[2]); }
	 ...
	 */
	gdt::vec3f getPower() const { return _power; }
	 ...
	 */
	void setLocation(const gdt::vec3f& loc) {
	    _position[0] = loc.x;
	    _position[1] = loc.y;
	    _position[2] = loc.z;
	}
