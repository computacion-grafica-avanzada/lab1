...
void PhotonMap::_build(const vector<Photon>& photons)
{
    ...
    for (register unsigned i = 0; i <= _storedPhotons; i++) {
		pa2[i] = &_map[i];
	}
...
vector<Photon*>* PhotonMap::getNeighbourhood(const vec3f& spoint,
					     const vec3f* snormal,
					     float          maxdist,
					     unsigned       nphotons) const 
{
	...
	if (snormal) {
		if (dot(*snormal, (*np)(i).getDirection()) > 0.) {
		ret->push_back(&(*np)(i));
	    }
	} else {
	    ret->push_back(&(*np)(i));
...
bool PhotonMap::getIrradiance(const vec3f& spoint,
			      const vec3f* snormal,
			      float          maxdist,
			      unsigned       nphotons,
			      vec3f&       irrad) const
{
    irrad = vec3f(0.0, 0.0, 0.0);
	...
	irrad * vec3f(1. / (3.14 * maxdist * maxdist));
    delete np;
    return true;
}
