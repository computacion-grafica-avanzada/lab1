...
namespace osc {
    ...
    /*! set lights to render with */
    void SampleRenderer::setLights(const std::vector<Light*> _lights)
    {
        lights = _lights;
        launchParams.lights = new Light2[lights.size()];
        for (int i = 0; i < lights.size(); i++) {
            PointLight* pointLight = static_cast<PointLight*>(lights[i]);
            launchParams.lights[i].position = pointLight->position;
            launchParams.lights[i].power = pointLight->power;
            launchParams.lights[i].numberPhotons = pointLight->numberPhotons;
        };
    }
