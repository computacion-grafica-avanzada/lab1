...
class PhotonMap {
	 ...
	 */
	virtual vector<Photon*>* getNeighbourhood(const vec3f& spoint,
						  const vec3f* snormal,
						  float          maxdist,
						  unsigned       nphotons
						  ) const;
	 ...
	 */
	virtual bool getIrradiance(const vec3f& spoint,
				   const vec3f* snormal,
				   float          maxdist,
				   unsigned       nphotons,
				   vec3f&       irrad) const;
...
class PhotonMap::NearestPhotons {
    ...
    public:
	NearestPhotons(unsigned n, const vec3f p, float maxDist,
	...
	vec3f pointLocation() const { return vec3f(pos[0],pos[1],pos[2]); }
