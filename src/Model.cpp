...
namespace osc {
  ...
  Model *loadOBJ(const std::string &objFile)
  {
    ...
    std::cout << materials[0].unknown_parameter.find("Pwr")->second;
    std::cout << (materials[1].unknown_parameter.count("Pwr") == 1 ? materials[1].unknown_parameter.find("Pwr")->second : "None");
    ...
    for (int shapeID=0;shapeID<(int)shapes.size();shapeID++) {
      ...
      std::cout << shape.name << shape.points.indices.size() << " " <<shape.mesh.material_ids.size() << " " << materialIDs.size() << std::endl;
      for (int materialID : materialIDs) {
        ...
        if (mesh->vertex.empty())
          delete mesh;
        /*else if (materials[materialID].unknown_parameter.count("Pwr") == 1) {
            Light* light = new Light;
            light->ambient = mesh->ambient;
            light->diffuse = mesh->diffuse;
            light->specular = mesh->specular;
            light->dissolve = mesh->dissolve;
            light->ior = mesh->ior;
            light->exponent = mesh->exponent;
            light->power = vec3f(std::stof(materials[materialID].unknown_parameter.find("Pwr")->second));
            light->color = vec3f(std::stof(materials[materialID].unknown_parameter.find("Pwr")->second));
        }*/ 
        else
